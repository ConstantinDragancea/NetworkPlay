...
def main():
    ...
    try:
        ...
        start_window = seq_nr + 1
        windowBuffer = dict()

        mutex = Lock()

        def ListenForAck():
            nonlocal start_window
            nonlocal window
            nonlocal windowBuffer
            nonlocal fileFinished

            global TIMEOUT

            while fileFinished is False or window > 0:
                try:
                    ...
                    segmentPos = r_ack_nr - start_window
                    if (segmentPos >= 0 and segmentPos < window and segmentConfirmed[segmentPos] == False):
                    ...
                    while len(segmentConfirmed) > 0 and segmentConfirmed[0] is True:
                        segmentConfirmed.pop(0)
                        segments.pop(0)
                        segmentSentTime.pop(0)
                        start_window += 1
                        window -= 1

                    if fileFinished is False:
                        window = max(window, r_window)
                    ...
                    logging.info("Timeout la asteptare pentru confirmare, retrying...")
                    mutex.acquire()
                    TIMEOUT = increaseTimeout(TIMEOUT)
                    sock.settimeout(TIMEOUT)
                    mutex.release()

        listener_thread = Thread(target=ListenForAck)
        ...
        while fileFinished is False or window > 0:
            ...
            # Mai citim din fisier in cazul cand s-a shiftat sau s-a marit window-ul
            for i in range(len(segments), window):
                segments.append(None)
                segmentConfirmed.append(False)
                segmentSentTime.append(None)
                if fileFinished is True:
                    window = i
                    break
                info = citeste_segment(file_descriptor)
                if len(info) == 0:
                    fileFinished = True
                    window = i
                    break
            ...
            # Iteram si trimitem sau retrimitem in caz de timeout
            for i in range(window):
                if (segmentSentTime[i] == None) or (not segmentConfirmed[i] and (datetime.now() - segmentSentTime[i]).total_seconds() > TIMEOUT):
                    packet = SenderPacket(data = segments[i], msg_type='P', seq_nr=start_window + i)
